...
ppo_atari_template = "env CUDA_VISIBLE_DEVICES={2} python main.py --env-name {0} --algo ppo --use-gae --lr 2.5e-4 --clip-param 0.1 --value-loss-coef 0.5 --num-processes 8 --num-steps 128 --num-mini-batch 4 --log-interval 1 --use-linear-lr-decay --use-linear-clip-decay --entropy-coef 0.01 --log-dir /tmp/gym/{1}/{1}-{2} --seed {2}"
...
for i in range(args.num_seeds):
    ...
    for env_name in args.env_names.split(';'):
        panes_list.append(template.format(env_name, env_name.split('-')[0].lower(), i))
